...
class AppStore extends Component {
  ...
  render() {
    ...
    const appDetail = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput),
    )
    const currentTabAppList = appDetail.filter(
      each => currentCategory === each.category,
    )
      ...
      <div className="container-bg">
        ...
        <div className="input-card">
          <div className="input-container">
            <input type="search" onChange={this.toChangeInput} />
            <img
        ...
        <ul className="tab-container">
          {tabsList.map(each => (
            <TabItem
              tabDetail={each}
              key={each.tabId}
              changeTabBtn={this.changeTabBtn}
              isCurrentTab={currentCategory === each.tabId}
            />
          ))}
        </ul>
        <div className="app-list-container-card">
          <ul className="app-list-container">
            {currentTabAppList.map(each => (
